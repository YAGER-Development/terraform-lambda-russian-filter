...
data "archive_file" "autoscale" {
  type        = "zip"
  source_file = ".${replace(path.module, path.root, "")}/lambda/russina_filter/autoscale.py"
...
resource "aws_lambda_function" "autoscale_handling" {
  ...
  description      = "Handles autoscaling groups with public IP's and terminate the instance if has a blacklisted one"
}
